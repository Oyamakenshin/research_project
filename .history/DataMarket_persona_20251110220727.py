...
class Participants(Agent):
    ...
    def initialize_attributes(self, persona):
        ...
        if self.persona == "intrinsic":
            # intrinsic型: 「独占すること自体」に価値を置く
            self.l_n_minus_1 = -self.random.normalvariate(50, 10)  # 他者保有の影響は小さめ
            wn_multiplier = self.random.uniform(0.70, 0.90)        # w_nはw_1の70〜90%程度
            self.w_n = self.w_1 * wn_multiplier
        
        elif self.persona == "follower":
            # follower型: 「みんなが持っている」ことを重視
            self.l_n_minus_1 = -self.random.normalvariate(300, 40) # 他者保有で効用が強く下がる
            wn_multiplier = self.random.uniform(0.2, 0.4)          # w_nはw_1の20〜40%程度
            self.w_n = self.w_1 * wn_multiplier
...
class DataMarket(Model):
    ...
    def __init__(self, num_agents, num_data, initial_price, base_wealth, 
        ...
        # personaの比率に応じて分類
        num_intrinsic = int(self.num_agents * self.persona_dist.get('intrinsic', 0))
        self.num_intrinsic = num_intrinsic
        self.num_follower = self.num_agents - num_intrinsic
        ...
        for i, agent in enumerate(agent_list):
            persona = "intrinsic" if i < num_intrinsic else "follower"
